"use strict";
// Use the dedicated smoke canvas if available. Fall back to first canvas.
let canvas = document.getElementById('smoke-canvas') || document.getElementsByTagName("canvas")[0];
// Ensure the canvas covers the viewport initially
function setCanvasSizeToViewport() {
  canvas.style.width = '100vw';
  canvas.style.height = '100vh';
  canvas.width = Math.max(window.innerWidth || 0, 1);
  canvas.height = Math.max(window.innerHeight || 0, 1);
}
setCanvasSizeToViewport();
window.addEventListener('resize', function(){ setCanvasSizeToViewport(); resizeCanvas(); }, {passive:true});
let config = {
...
function resizeCanvas() {
  // Keep canvas sized to viewport. If changed, update framebuffers.
...
// Listen on window so canvas can be non-interactive (pointer-events:none) while still reacting.
...
window.addEventListener('touchmove', function (e) {
  // Allow page scrolling by not calling preventDefault here; we still capture positions.
  ...
  for (let i = 0, len = touches.length; i < len; i++) {
    ...
    pointers[i].id = touches[i].identifier || i;
